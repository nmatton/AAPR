...
### Epic 4.2: Practice Recommendations on Issues 

This epic pulls forward the practice recommendation feature into the MVP scope. It leverages the affinity scoring foundation built in Epic 4.1 to provide teams with actionable alternatives when an issue highlights friction with a current practice. 

**Dependencies:** Epic 4.1 (Affinity Scoring Foundation) and Epic 3 (Issue & Discussion)
**Goal:** Provide an automated, coverage-aware, and affinity-aware recommendation engine and display it on the Issue Detail page contextually.

#### Story 4.2.1: Recommendation Engine Service & API Endpoint

- **Description:** Implement the backend recommendation algorithm combining coverage parity with affinity score increases.
- **Goal:** Expose an API endpoint that takes a team ID and a target practice ID, and returns up to 3 recommended alternative practices.
- **Acceptance Criteria:**
  - `GET /api/teams/:id/practices/:practiceId/recommendations` endpoint created and guarded.
  - Logic filters out practices that reduce overall team agile coverage.
  - Logic filters out practices where `team_affinity_score <= current_practice.team_affinity_score` (requires strictly higher affinity).
  - Logic prioritizes practices of the same `type` or `category`.
  - Logic heavily weights practices linked to the target via `practice_associations` with `association_type = 'Equivalence'`.
  - Endpoint returns a maximum of 3 results.
- **Technical Notes:** Implement as a TypeScript service (`RecommendationService`) querying the existing `practice_associations` and `team_coverage` data. Ensure test coverage for the priority weighting rules.

#### Story 4.2.2: Practice Recommendations Widget (UI)

- **Description:** Implement the `RecommendationWidget` in the Issue Detail sidebar to surface the engine's suggestions.
- **Goal:** Non-intrusively offer alternative practices right where the team is discussing friction.
- **Acceptance Criteria:**
  - Sidebar on the Issue Detail view includes a new section: "Alternative Practices".
  - Fetches and displays up to 3 practices using the endpoint from Story 4.2.1.
  - Each listed practice displays its name, the affinity score difference (e.g., "+0.2 Affinity"), and a brief rationale string generated by the frontend or backend (e.g., "Covers the same pillars, higher team fit").
  - Clicking a recommendation opens a modal or navigates to the Practice Catalog to view its full details.
  - Handles loading states, and hides gracefully if no recommendations exist or if the endpoint fails.
- **Technical Notes:** Build as an isolated React/Vue component. Use optimistic UI or SWR/React Query for smooth fetching.

...
- [ ] Task 1: Baseline production runtime contracts (AC: 1, 2, 3)
  - [ ] Subtask 1.1: Confirm current backend runtime entrypoint and compiled output path from `server/package.json` and `server/tsconfig.json`.
  - [ ] Subtask 1.2: Confirm current frontend production build output and static serving strategy from `client/package.json` and Vite config.
  - [ ] Subtask 1.3: Define minimal required runtime env contract for each service (backend mandatory, frontend deployment-bound variables).

- [ ] Task 2: Create backend production Dockerfile (AC: 1, 2, 4)
  - [ ] Subtask 2.1: Add a multi-stage backend Dockerfile using pinned Node LTS base and deterministic install (`npm ci`).
  - [ ] Subtask 2.2: Build TypeScript output in build stage; copy only runtime artifacts and production dependencies into final stage.
  - [ ] Subtask 2.3: Configure production startup command and internal service port exposure aligned with Epic 7 port contract.
  - [ ] Subtask 2.4: Add backend container healthcheck strategy compatible with planned pipeline smoke checks.

- [ ] Task 3: Create frontend production Dockerfile (AC: 1, 3, 4)
  - [ ] Subtask 3.1: Add a multi-stage frontend Dockerfile (build static bundle, serve with production web server layer).
  - [ ] Subtask 3.2: Ensure frontend runtime image only contains built assets and required serving config.
  - [ ] Subtask 3.3: Define the frontend internal container port and startup contract for compose integration.
  - [ ] Subtask 3.4: Document how frontend runtime resolves backend API endpoint in multi-instance deployments.

- [ ] Task 4: Add local verification and smoke checks for image validity (AC: 1, 2, 3, 5)
  - [ ] Subtask 4.1: Build both images locally with reproducible commands from repository root.
  - [ ] Subtask 4.2: Run backend container and verify health endpoint behavior.
  - [ ] Subtask 4.3: Run frontend container and verify static app serving plus API proxy/base URL behavior.
  - [ ] Subtask 4.4: Capture the exact validation commands for reuse in Story 7.6 and 7.7 pipeline steps.

- [ ] Task 5: Documentation and handoff alignment (AC: 5)
  - [ ] Subtask 5.1: Update `docs/07-infrastructure.md` with container build/run instructions and production notes.
  - [ ] Subtask 5.2: Update `docs/08-development-guide.md` with local validation workflow and Docker usage boundaries.
  - [ ] Subtask 5.3: Add story completion entry to `docs/09-changelog.md` once implementation is finished.
...
- Generated a dedicated implementation-ready story artifact for Epic 7 Story 7.1.
- Added technical, architectural, file-structure, and testing constraints to prevent deployment-regression mistakes.
- Prepared handoff context for Story 7.2+ compose and CI/CD pipeline work.

...
const buildInviteEmail = (payload: InviteEmailPayload) => {
  ...
  const subject = `You're invited to join ${payload.teamName} on Addaptive Agile Practices Repository (AAPR)`
  const text = `You've been invited to join the team '${payload.teamName}' on Addaptive Agile Practices Repository (AAPR). Please sign up using this email address to access the platform.\n\nCreate Account: ${payload.ctaUrl}`
  const html = `
    <p>You've been invited to join the team '<strong>${escapedTeamName}</strong>' on Addaptive Agile Practices Repository (AAPR).</p>
...
const buildAddedEmail = (payload: InviteEmailPayload) => {
  const escapedTeamName = escapeHtml(payload.teamName)
  const subject = `You've been added to the team ${payload.teamName} on Addaptive Agile Practices Repository (AAPR)`
  const text = `You've been added to the team '${payload.teamName}' on Addaptive Agile Practices Repository (AAPR). Click the link to access it now.\n\nView Team: ${payload.ctaUrl}`
  const html = `
    <p>You've been added to the team '<strong>${escapedTeamName}</strong>' on Addaptive Agile Practices Repository (AAPR).</p>
